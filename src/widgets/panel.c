...
void ti_render_panelw(termi_state *termi, termi_widget *widget) {
  ...
  if (panelw->draw_box) {
    for (int i = 0; i < widget->width; i++) {
      ti_nset_cellrc(termi, widget->row, widget->col + i, ' ', -1, 0);
      ti_nset_cellrc(termi, widget->row + widget->height + panelw->padding - 1, widget->col + i, ' ', -1, 0);
    }

    for (int i = 0; i < widget->height + 1; i++) {
      ti_nset_cellrc(termi, widget->row + i, widget->col, ' ', -1, 0);
      ti_nset_cellrc(termi, widget->row + i, widget->col + widget->width - panelw->padding + 1, ' ', -1, 0);
    }
  }

  if (panelw->draw_title) {
    panelw->title->widget.abs_row = widget->row;
    panelw->title->widget.abs_col = widget->col + 1;

    panelw->title->bg = 0;

    ti_render_widget(termi, (termi_widget *)panelw->title);
  }
